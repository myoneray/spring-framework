 ...
 * {@code web.xml} context-param level to specify the context class type,
 * falling back to
 * {@link org.springframework.web.context.support.XmlWebApplicationContext} if
 * not found. With the default ContextLoader implementation, any context class
 * specified needs to implement the {@link ConfigurableWebApplicationContext}
 * interface.
 *
 * <p>
 * Processes a {@link #CONFIG_LOCATION_PARAM "contextConfigLocation"}
 ...
 * "WEB-INF/applicationContext1.xml, WEB-INF/applicationContext2.xml". Ant-style
 * path patterns are supported as well, e.g.
 * "WEB-INF/*Context.xml,WEB-INF/spring*.xml" or
 * "WEB-INF/&#42;&#42;/*Context.xml". If not explicitly specified, the context
 ...
 * application context. See the {@link #loadParentContext(ServletContext)}
 * method for more information.
 *
 * <p>
 * As of Spring 3.1, {@code ContextLoader} supports injecting the root web
 * application context via the {@link #ContextLoader(WebApplicationContext)}
 * constructor, allowing for programmatic configuration in Servlet 3.0+
 * environments. See {@link org.springframework.web.WebApplicationInitializer}
 * for usage examples.
 *
...
public class ContextLoader {
    ...
    /**
     * Config param for the root WebApplicationContext id, to be used as
     * serialization id for the underlying BeanFactory: {@value}
     */
    public static final String CONTEXT_ID_PARAM = "contextId";

    /**
     * Name of servlet context parameter (i.e., {@value} ) that can specify the
     * config location for the root context, falling back to the
     * implementation's default otherwise.
     * 
    ...
    /**
     * Config param for {@link ApplicationContextInitializer} classes to use for
     * initializing the root web application context: {@value}
     * 
    ...
    /**
     * Config param for global {@link ApplicationContextInitializer} classes to
     * use for initializing all web application contexts in the current
     * application: {@value}
     * 
     * @see #customizeContext(ServletContext, ConfigurableWebApplicationContext)
     */
     ...
     * ") used only when obtaining a parent context using the default
     * implementation of
     ...
     * <p>
     * The default is {@code classpath*:beanRefContext.xml}, matching the
     * default applied for the
     ...
     * instance.
     * <p>
     * Supplying this "parentContextKey" parameter is sufficient when relying on
     * the default {@code classpath*:beanRefContext.xml} selector for candidate
     * factory references.
     */
    ...
    /**
     * Create a new {@code ContextLoader} that will create a web application
     * context based on the "contextClass" and "contextConfigLocation" servlet
     * context-params. See class-level documentation for details on default
     * values for each.
     * <p>
     * This constructor is typically used when declaring the
     * {@code ContextLoaderListener} subclass as a {@code <listener>} within
     * {@code web.xml}, as a no-arg constructor is required.
     * <p>
     ...
     * subclasses are free to call the {@link #closeWebApplicationContext}
     * method on container shutdown to close the application context.
     * 
     ...
     * {@linkplain ConfigurableApplicationContext#refresh() refreshed}. If it
     * (a) is an implementation of {@link ConfigurableWebApplicationContext} and
     * (b) has <strong>not</strong> already been refreshed (the recommended
     * approach), then the following will occur:
     * <ul>
     * <li>If the given context has not already been assigned an
     * {@linkplain ConfigurableApplicationContext#setId id}, one will be
     * assigned to it</li>
     ...
     * <li>{@link ConfigurableApplicationContext#refresh refresh()} will be
     * called</li>
     * </ul>
    ...
    /**
     * Specify which {@link ApplicationContextInitializer} instances should be
     * used to initialize the application context used by this
     * {@code ContextLoader}.
     * 
     * @since 4.2
     ...
     * creating a new one according to the "{@link #CONTEXT_CLASS_PARAM
     * contextClass}" and "{@link #CONFIG_LOCATION_PARAM contextConfigLocation}"
     * context-params.
     * 
    ...
    public WebApplicationContext initWebApplicationContext(ServletContext servletContext) {
        ...
        // 判斷servletContext中是否已经注册了WebApplicationContext
    ...
    protected Class<?> determineContextClass(ServletContext servletContext) {
        /**
         * 首先determineContextClass()方法查明具体的Context类.
         * 读取servletContext的初始化参数contextClass
         */
        ...
        if (contextClassName != null) {
            try {
                /**
                 * 所以Spring就会读取跟
                 * org.springframework.web.context.WebApplicationContext <br>
                 * 同一个包下面的ContextLoader .properties文件读取默认设置， <br>
                 * 反射出org .springframework.web.context.support.
                 * XmlWebApplicationContext类来
                 */
    ...
    protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac, ServletContext sc) {
        if (ObjectUtils.identityToString(wac).equals(wac.getId())) {
            ...
            // 将新创建的XmlWebApplicationContext进行初始化。
            String idParam = sc.getInitParameter(CONTEXT_ID_PARAM);
            if (idParam != null) {

                // 首先会设置一个默认ID，即org.springframework.web.context.WebApplicationContext:+你项目的ContextPath。
                wac.setId(idParam);
            } else {
        ...
        /**
         * 接下来就是读取web.xml文件中的contextConfigLocation参数。
         * 如果没有配置就会去读WEB-INF下的applicationContext.xml文件。
         * 并将值设置(就是我们的Spring配置文件的路径)进XmlWebApplicationContext中。
         */
        ...
        /**
         * 接下来就是customizeContext(sc,wac)方法，
         * 此方法会根据用户配置的globalInitializerClasses参数来初始化一些用户自定义的属性，
         * 一般我们不配置，所以这里什么也不做。
         */
     ...
     * {@linkplain #determineContextInitializerClasses(ServletContext)
     * determines} what (if any) context initializer classes have been specified
     * through {@linkplain #CONTEXT_INITIALIZER_CLASSES_PARAM context init
     * parameters} and {@linkplain ApplicationContextInitializer#initialize
     * invokes each} with the given web application context.
     * <p>
    ...
    /**
     * Return the {@link ApplicationContextInitializer} implementation classes
     * to use if any have been specified by
     ...
     * {@link #LOCATOR_FACTORY_KEY_PARAM}, to load a parent context which will
     * be shared by all other users of ContextsingletonBeanFactoryLocator which
     * also use the same configuration parameters.
     * 
     * @param servletContext current servlet context
     ...
     * <p>
     * If overriding {@link #loadParentContext(ServletContext)}, you may have to
     * override this method as well.
     * 
     * @param servletContext the ServletContext that the WebApplicationContext
     *            runs in
     */
     ...
     * (i.e. for the current thread's context ClassLoader, which needs to be the
     * web application's ClassLoader).
     * 
     * @return the current root web application context, or {@code null} if none
     *         found
